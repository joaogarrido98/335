...
def guess(letter):
	...
	if guesses<11:
		...
		if the_word_withSpaces.count(letter)>0:
			...
			guesses += 1
			imgLabel.config(image=images[guesses])
			Main.showMessage(guesses)
...
lblWord = StringVar()
Label(window, textvariable  =lblWord,font=('consolas 30 bold')).grid(row=0, column=3 ,columnspan=6,padx=10)
